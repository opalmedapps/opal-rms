...
# Example:
#http://172.26.66.41/devDocuments/devscreens/php/fillMediVisitAppointment.php?PatientId=12345&Ramq=ABCDEF123456&RamqExpireDate=0516&PatLastName=XXX12&PatFirstName=John&ResourceCode=NSBLD&ResourceName=NS%20-%20prise%20de%20sang/blood%20tests%20pre/post%20tx&AppointDate=01/06/2017&AppointTime=10:50&AppointCode=BLD-XY&Status=Open&Action=DEL&AppointId=13&AppointSys=Medivisit&verbose=0
...
$SSN			= $_GET["Ramq"];
$SSNExpDate		= $_GET["RamqExpireDate"];
$Resource		= $_GET["ResourceCode"];
$ResourceDesc		= $_GET["ResourceName"];
$ScheduledDate 		= $_GET["AppointDate"];
$ScheduledTime  	= $_GET["AppointTime"];
$AppointmentCode	= $_GET["AppointCode"];
$Status 		= $_GET["Status"];
$Action			= $_GET["Action"];
$AppointId 		= $_GET["AppointId"];	
$AppointSys		= $_GET["AppointSys"];	
...
# If the AppointIdIn is InstantAddOn then we need to create an appointment ID on the fly
# using the patient's LastName and the datetime
if($AppointIdIn == "InstantAddOn")
{
  $AppointId = "$SSN-$ScheduledDate-$ScheduledTime";
  
  if($verbose)
  {
    echo "Instant Add-on with Id: $AppointId<br>";
  }
...
# Check the status - true appointments come in as A, cancelled *visits* come in as C
$AppointStatusLetter    = substr($AppointIdIn, 4, 1); # the status letter comes after the creation year
if($AppointStatusLetter == "C")
{
  $Status = "Deleted";

  # Snce the AppointId after the C is actually the new visit Id, we should search
  # the ORMS database to find the true appointment Id, and set its status as "Deleted" 
...
if($AppointSys != "Medivisit" && $AppointSys != "InstantAddOn")
{
...
# 1) ADD - add new appointment and possibly add new patient
# 2) UPD - update appointment and possibly update patient
# 3) DEL - delete appointment (treat as an update except with Status = "Delete") and possibly update patient
#==========================================================================================
...
if($AppointIdIn == "InstantAddOn")
{
  echo "<h1>Add-on Appointment Created in ORMS</h1>";
  #echo "<h2>Please tell patient to check-in at kiosk</h2>";


  #$CheckInURL_raw = "./checkInPatientAriaMedi.php?CheckinVenue=ADDED ON BY RECEPTION&PatientId=$PatientId";
  $CheckInURL_raw = "$baseURL/php/checkInPatientAriaMedi.php?CheckinVenue=ADDED ON BY RECEPTION&PatientId=$PatientId";
  ...
  #echo "CheckInURL: $CheckInURL<br>";
  $lines = file($CheckInURL);

  echo "<h2>Patient has been checked in for this appointment. You may tell the patient to wait in the waiting room.</h2>";

  echo "<b>PatientId:</b> $PatientId	<br>";
  echo "<b>LastName:</b> $LastName<br>";
  echo "<b>FirstName:</b> $FirstName<br>";	
  echo "<b>SSN:</b> $SSN	<br>";	
  echo "<b>SSNExpDate:</b> $SSNExpDate	<br>";
  echo "<b>Resource:</b> $Resource	<br>";
  echo "<b>ResourceDesc:</b> $ResourceDesc	<br>";
  echo "<b>ScheduledDate:</b> $ScheduledDate <br>";
  echo "<b>ScheduledTime:</b> $ScheduledTime  <br>";
  echo "<b>AppointmentCode:</b> $AppointmentCode<br>";
  echo "<b>Status:</b> $Status 	<br>";
  echo "<b>Action:</b> $Action	<br>";
  echo "<b>AppointId:</b> $AppointId 	<br>";
  echo "<b>AppointSys:</b> $AppointSys	<br>";
